...
BAC0 is a Python 3 (3.5 and over) scripting application that uses BACpypes_ to process BACnet messages on an IP network. 
This library offers simple commands to browse a BACnet network, read properties from BACnet devices or write to them.
...
=====================
BAC0 includes a local web page that will help the user providing basic information about the network seen by the script and also provide a simple interface to historical trends. Flask is used to render the web page and a Bokeh server is also provided to serve live trends to the user.
...
BAC0, is a unified way, using the Python language and BACnet/IP communication, to interact with those controllers once their sequence is built.
...
property, it's easy to write a value to the input so the controller will think an input is connected. 
...
every test must be done manually. That means also that if you want to repeat the tests, the more complicated they are, the less likely you'll be able to do so.

Now you can write your tests and run them as often as you want. We'll show you how it works.
...
every variable in the controller. Each point is trended every 10 seconds by default. Which means 
